...
export const PReimbursements: React.FC = () => {
    ...
    const getAllReimbursements = async () => {
        const reimbursementData = await axios.get("http://localhost:8080/reimbursements/pending", {withCredentials:true}
        )
    ...
    const approve = (reimbId:number) => async () => {
        await axios.patch(`http://localhost:8080/reimbursements/status/${reimbId}`, {status: 'Approved'}, { withCredentials: true });
            ...
            <Table className = "table-primary table-hover">
                ...
                {reimbursements.map((reimbursement:Reimbursement) => (
                        <tr key={reimbursement.reimbId}>
                            ...
                            <td>{reimbursement.status === "pending" ? <><button onClick={approve(reimbursement.reimbId)}>✔️</button><button onClick={approve(reimbursement.reimbId)}>❌</button></> : null}</td>
                        </tr>
                    ))}
                </tbody>
            </Table>
